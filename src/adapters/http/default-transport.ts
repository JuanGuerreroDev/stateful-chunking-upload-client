 ...
 * `globalThis.fetch` (nativo en navegador y Node ≥18).
 *
...
export function resolveTransport(injected?: Transport): Transport {
  ...
  if (typeof globalFetch !== 'function') {
    throw new TransportUnavailableError(
      'No se inyectó Transport y no hay fetch global disponible; provee uno explícito (Node <18 requiere un polyfill).',
    );
  }
